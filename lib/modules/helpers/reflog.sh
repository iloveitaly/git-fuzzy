...
gf_helper_reflog_inspect() {
  ...
  if [ "$BASE" == "$REF" ]; then
    MERGE_BASE="$(git merge-base "$GF_BASE_BRANCH" "$REF")"
    {
      ...
      # shellcheck disable=2086
      gf_git_command_with_header_default_parameters 1 "$GF_DIFF_COMMIT_RANGE_PREVIEW_DEFAULTS" diff "$MERGE_BASE..$REF" $QUERY |
        gf_helper_inspect_diff_renderer
    } | gf_helper_inspect_pager
  else
    # shellcheck disable=2086
    gf_git_command_with_header_default_parameters 1 "$GF_DIFF_COMMIT_RANGE_PREVIEW_DEFAULTS" diff "$BASE" "$REF" $QUERY |
      gf_helper_inspect_diff_renderer |
      gf_helper_inspect_pager
  fi
}
